...
class Game:
    ...
    def __init__(self, screen_size=(600, 600), board_size=(18, 18)):
        ...
        # Сохраняем размер как число (длина стороны доски)
        self.size = board_size[0]
        self.board_size = board_size

        # Инициализируем prisoners и turn, чтобы не было ошибок в RL и методах capture_stones
        self.prisoners = {'black': 0, 'white': 0}
        self.black_turn = True

        # Изначально создаем доску нулями через numpy, так как методы используют .shape
        import numpy as np
